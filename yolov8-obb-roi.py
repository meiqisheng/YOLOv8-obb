...
if __name__ == "__main__":
    ...
    imgpath = "E:/ganzhoudata/test2/roi/"
    results = model.predict(source = imgpath,save =True,save_txt = True)
    # resultpath = "E:/ganzhoudata/test2/result/"
    # for filename in os.listdir(imgpath):
    #     (txtname, suffix) = os.path.splitext(imgpath  + filename)
    #     if((suffix != ".jpeg") or (txtname != ".jpg")):
    #         continue
    ...
    #     (txtname, suffix) = os.path.splitext(filename)
    #     txtname = imgpath + txtname + ".txt"
    ...
    #             writeline = str(label) + " " + str(confidence) + " " + str(left/(width*1.0)) +  " " + str(top/(height*1.0)) +  " " + str(w/(width*1.0)) +  " " + str(h/(height*1.0)) + "\n" 
    #             f.writelines(writeline)
    #     f.close()
        
    #     savename = resultpath + "predict-" + filename

    #     cv2.imwrite(savename, img)
    #     print("save done" + savename)
    # else:
    #     print("image error")    
